...
def assemble_probabilities(mat, num_range):
	"""
...
def create_tree(mat, num_range):
	...
	matrix_subs = matrix_substitutions(mat, num_range) #List of subsitution vectors.
...
def matrix_substitutions(mat, num_range):
	"""
	...
	"""
	row_subs = matrix_combinations(mat, num_range)
	mat_subs = matrix_substitution_helper(row_subs)
	
	matrix_subs = []
	for sub in mat_subs:
		matrix_subs.append(list(sub))
	
	final_matrix_subs = []
	for sub in matrix_subs:
		final_matrix_subs.append(matrix_to_vector(sub))
...
def matrix_combinations(mat,num_range):
	"""
	...
	for i in range(0,n,2):
		pair = [vec[i],vec[i+1]]
		matrix_subs.append(pair_substitution(pair,num_range))
		
	return matrix_subs
	
#########################################################################################

def pair_substitution(pair, num_range):
	"""
	RETURNS: A list of all possible substitutions for the given
			pair of numbers. (list of lists)
			
	PARAMETERS: pair(list): Given pair of numbers to substitute.
				num_range(list): The range of numbers that can be
								substituted in the matrix in the format [min,max].
								Eg: [0,255] for RGB.
								
	EXAMPLE: Input: pair =[0,2], num_range = [0,2] ; Output: [[0,2],[1,1],[2,0]]
			Input: [0,0], num_range = [0,10] ; Output: [[0,0]]
	"""
	
	target = pair[0] + pair[1] #Sum of numbers to be substituted.
	min = num_range[0]
	max = num_range[1]
	numbers = range(min, max+1) #Numbers that can be substituted.
	
	list_subs = [] #Contains all possible substitutions for the pair.
	
	for num in numbers:
		other_num = target - num
		if other_num in numbers:
			list_subs.append([num, other_num])
